...
for sj = 1:length(subj_list) % loop through subjects
    ...
    for se = 1:se_total % loop through sessions
        ...
        for bl = 1:num_blocks
            ...
            responses_clean = responses{bl}(~isnan(responses{bl}(:,1)),:); % remove nan rows from matrix - used for speed allocation during task
            keyboard;
%%%%%%%%%%%%% figure out coherence of missed trials
              
            missed_trials_idx = responses_clean(:,7) == 3; % get idx of missed trials 
            
            frame_missed = responses_clean(missed_trials_idx,6); % get frame of missed trials 
                
            coherence_missed = stim.S.mean_coherence_org{bl}(frame_missed - 51); % get coherence from mean stim tream - go 51 frames back because subjects have until 0.5 sec after trial to respond before it is missed

            responses_clean(missed_trials_idx,4) = coherence_missed; % insert coherences of missed trials in response matrix 
%%%%%%%%%%%%%

            
            block = str2double(stim.S.block_ID_cells{bl}); % get condition ID
            
            num_responses = length(responses_clean(:,1)); % get num of butten presses and missed trials
...
%                          trial version
            all_responses(idx_block_start:idx_block_start + num_responses-1,1:7) = responses_clean(:,1:7);
            ...
            switch which_data
                ...
                case 'main experiment'
                    ...
                    trigger_streams{sj,se}(:,block) = behav_stim_trig.trigger_vals{bl};
                ...
                case 'short trials pilot'
                    ...
                    trigger_streams{sj,se}(:,bl) = behav_stim_trig.trigger_vals{bl};
