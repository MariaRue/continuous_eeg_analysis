...
for coh = 1:3
    ...
    y_level = [0.6 0.55 0.6];
    for id = 1:2
    if any(sig_tps{id})
plot(sig_tps{id},ones(length(sig_tps{id}),1).*  y_level(id),'k*')
...
plot(1:501,zeros(501,1),'k--','LineWidth',3)
hold on 
y_level = [0.5 0.55 0.6];
for id = 1:3
    if any(sig_tps{id})
plot(sig_tps{id},ones(length(sig_tps{id}),1).* y_level(id),'k*','MarkerSize',2)
...
% regressors combined 
reg_comb = [freq_reg len_reg interact_reg];
