...
lambda = 17.6; %4 * value used in Gonclaves et al 2014 (as our regularisation matrix is same as theirs, divided by 4)
vertical = 0;
...
for subject = 1:length(subjectList)
    ...
    if tfDecomp
        subjectMatchedEEGdata = load(paths.(reference).matchedEEG_tf.saveName);
    else
