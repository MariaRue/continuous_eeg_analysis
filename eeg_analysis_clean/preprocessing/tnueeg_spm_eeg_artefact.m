function D = tnueeg_spm_eeg_artefact(S)
%TNUEEG_SPM_EEG_ARTEFACT Modified SPM function:
...
%
% sdThresholding (tnueeg_spm_eeg_artefact_sdthresh) - thresholds EOG
%                                                   relative to its SD
%
% ampThresholding (tnueeg_spm_eeg_artefact_ampthres) - thresholds EOG at a
%                                                   given amplitude
%__________________________________________________________________________
...
if isequal(S.mode, 'reject')
    ...
    for i = 1:numel(S.methods)
        ...
        if ~isempty(chanind)
            ...
            bad = bad | feval(['tnueeg_spm_eeg_artefact_' S.methods(i).fun], S1);
        end
    end
    ...
    for i = 1:numel(S.methods)
        ...
        if ~isempty(chanind)
            ...
            if isfield(S, 'windowOfInterestInSamples')
                S1.windowOfInterestInSamples = S.windowOfInterestInSamples;
            end            
            
            D =  feval(['tnueeg_spm_eeg_artefact_' S.methods(i).fun], S1);
        end
    end    
