function res = tnueeg_spm_eeg_artefact_eyeblinks_for_correction(S)
%TNUEEG_SPM_EEG_ARTEFACT_EYEBLINKS_FOR_CORRECTION Modified SPM function:
...
% determine start and end of experimental session (only detecting eyeblinks there)
if isfield(S, 'windowOfInterestInSamples')
    ev = D.events;
    startSample = indsample(D, ev(S.windowOfInterestInSamples(1)).time) - 2*D.fsample;
    if startSample < 1
        warning('First sample of interest is too early. Starting with the first sample!');
        startSample = 1;
    end
    if numel(S.windowOfInterestInSamples) > 1
        endSample = indsample(D, ev(S.windowOfInterestInSamples(2)).time) + D.fsample;
    else 
        endSample = indsample(D, ev(end).time) + D.fsample;
        if endSample > nsamples(D);
            endSample = nsamples(D);
        end
    end
    eog_data = reshape(squeeze(D(chanind,startSample:endSample,:)), 1, []);
else
...
for i = eblength:length(eog_filt)-eblength;
    if eog_filt(i)>em_thresh && ... %bigger than threshold
       all(eog_filt(i)>=eog_filt(i-eblength+1:i+eblength)); %biggest in surrounding 400ms
        spikes = [spikes i];
    end
end
...
% only apply this for small peaks (< mean - 1 s.d.), to prevent detection of noise peaks, but keep
% large blinks ( > mean + 1 s.d.) and do precleaning later
...
%----------------------------------------------------------------------
if ~isempty(spikes)
    % check whether we reduced the EOG signal to the window of interest
    if exist('startSample', 'var')
        % if yes, then we need to shift the spike samples by the first sample(-1)
        spikes = spikes + startSample - 1;
    end
