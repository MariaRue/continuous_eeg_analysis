function [blockStartEEGIdx, blockEndEEGIdx] = ...
    crdm_eeg_find_best_match_eeg_behav_triggers(eegTriggers, bhvTriggers, D)
%MATCH_EEG_DATA_WITH_BEHAVIOURAL_TRIGGERS

blocklength = eegTriggers(end).time-eegTriggers(1).time; % length of block, in seconds
nSamplesEEG = round(blocklength*D.fsample)+1; % number of samples in block, at 100 Hz (fsample)

% INSTEAD OF ROUND, WE COULD USE SPM'S INDSAMPLE FUNCTION HERE?

% make a vector from the eeg trigger channel that corresponds to S.trigger_vals 
% (with 0s where no trigger occurs)
trigger_vals_eeg = zeros(nSamplesEEG, 1);
for s = 1: length(eegTriggers)
    trigger_vals_eeg(round((eegTriggers(s).time - eegTriggers(1).time)*D.fsample)+1) = ...
        eegTriggers(s).value;
end

% trigger values from behaviour
nSamplesBehav = length(bhvTriggers);
...
for c = 1: (nSamplesEEG_zp + 1- nSamplesBehav)
    nMatch(c) = sum(bhvTriggers == trigger_vals_eeg_zp(c: c+ nSamplesBehav -1));   
end
...
blockStartEEGIdx = findc(D.time, eegTriggers(1).time) - zp_size + best_match;
%
